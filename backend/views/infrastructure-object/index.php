...
/* @var $searchModel backend\models\InfrastructureObjectSearch */
...
$columns = [
    ...
    [
        ...
        'value' => function($model) {
            return Html::a($model->title_ru, ['update', 'id' => $model->id]);
        }
    ],
    [
        'attribute' => 'title_ua',
        'format' => 'raw',
        'value' => function($model) {
            return Html::a($model->title_ua, ['update', 'id' => $model->id]);
        }
    ],
    [
        'attribute' => 'category_number',
        'format' => 'raw',
        'value' => function($model) {
            $label = '-';

            if($model->category_number) {
                $label = Yii::$app->params['infrastructure_object_category'][$model->category_number];
            }
    ...
    [
        ...
        'attribute' => 'status',
        'label' => 'Статус',
        'trueLabel' => InfrastructureObject::STATUS_ENABLE,
        'falseLabel' => InfrastructureObject::STATUS_DISABLE,
    ],
    [
